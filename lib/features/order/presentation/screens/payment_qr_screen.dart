...
class _PaymentQrScreenState extends State<PaymentQrScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<OrderBloc>().add(const OrderPaymentQrRequested());
    });
  }
      ...
      listeners: [
        BlocListener<OrderBloc, OrderState>(
          ...
          listener: (context, state) {
            ...
            if (state.failure!.message.contains('unavailable')) {
              context.pop();
            }
          },
        ),
        BlocListener<OrderBloc, OrderState>(
          listenWhen: (previous, current) =>
              current.paymentQr != null &&
              current.paymentQr != previous.paymentQr,
          listener: (context, state) =>
              _startCountdown(state.paymentQr!.secondsRemaining),
        ),
      ],
                        ...
                        children: [
                          ...
                          OutlinedButton.icon(
                            onPressed: orderState.isLoading
                                ...
                                : () => context
                                    .read<OrderBloc>()
                                    .add(const OrderPaymentQrRequested()),
